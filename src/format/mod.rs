...
pub fn input_with_dictionary<P: AsRef<Path>>(
    ...
    unsafe {
        let mut ps = ptr::null_mut();
        let path = from_path(path);
        let res = avformat_open_input(&mut ps, path.as_ptr(), ptr::null_mut(), &mut options.as_mut_ptr());
...
pub fn output_with<P: AsRef<Path>>(path: P, options: &mut Dictionary) -> Result<context::Output, Error> {
    unsafe {
        let mut ps = ptr::null_mut();
        let path = from_path(path);
